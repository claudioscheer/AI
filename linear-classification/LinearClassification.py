...
def high_loss(images, weights, bias):
    sum_loss = 0
...
# total_loss = high_loss(data_1, weigths, bias)
# print(total_loss)

loss_sum = 0
for x, y in zip(data_1[b"data"], data_1[b"labels"]):
    x_normalized = x / np.linalg.norm(x)
    scores = predict(weigths, x_normalized, bias)
    scores_softmax = softmax(scores)
    loss_sum += loss_log_likelihood(scores_softmax[y])

print(loss_sum / images_per_batch)
